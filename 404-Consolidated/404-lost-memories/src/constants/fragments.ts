...
export function getFragments(): MemoryFragment[] {
  if (typeof window === "undefined") return STATIC_MEMORY_FRAGMENTS;
  try {
    const stored = localStorage.getItem("lost_memories_dynamic");
    if (stored) {
...
export function getFragmentById(id: string): MemoryFragment | undefined {
  return getFragments().find(frag => frag.id === id);
}

export function saveDynamicFragment(fragment: MemoryFragment) {
  if (typeof window === "undefined") return;
  try {
    const current = localStorage.getItem("lost_memories_dynamic");
    const list: MemoryFragment[] = current ? JSON.parse(current) : [];
    list.push(fragment);
    localStorage.setItem("lost_memories_dynamic", JSON.stringify(list));
  } catch (e) {
