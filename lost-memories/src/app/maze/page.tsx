...
export default function MazePage() {
  ...
  // Load custom and static fragments on mount, scoped to current logged in user
  useEffect(() => {
    setFragments(getFragments(currentUser?.username));
  }, [currentUser]);
  ...
  const handleInjectPipeline = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      saveDynamicFragment(newFragment, currentUser?.username);
      setFragments(getFragments(currentUser?.username));
  ...
  return (
    ...
    >
      ...
      <div className="relative z-30 w-full max-w-5xl flex items-center justify-between mb-8 pb-4 border-b border-sky-900/10">
      ...
      >
        <motion.div
          animate={{ x: ["0%", "-15%", "8%", "0%"], y: ["0%", "15%", "-8%", "0%"], rotate: [0, -90, -180, -360], scale: [1, 1.15, 0.9, 1] }}
          transition={{ duration: 28, repeat: Infinity, ease: "easeInOut" }}
          style={{ willChange: "transform" }}
          className="absolute w-[120vw] h-[120vw] max-w-[1200px] max-h-[1200px] bg-[radial-gradient(circle_at_center,#082032,#02080d)] blur-[140px] bottom-[-30%] right-[-20%] opacity-70 rounded-full"
        />
        <motion.div
          animate={{ x: ["0%", "20%", "-12%", "0%"], y: ["0%", "-20%", "12%", "0%"], rotate: [360, 270, 90, 0], scale: [1, 0.85, 1.2, 1] }}
          transition={{ duration: 35, repeat: Infinity, ease: "easeInOut" }}
          style={{ willChange: "transform" }}
          className="absolute w-[100vw] h-[100vw] max-w-[1000px] max-h-[1000px] bg-[radial-gradient(circle_at_center,#0f304a,#030c14)] blur-[130px] top-[-40%] left-[-30%] opacity-60 rounded-full"
        />
      </motion.div>
      ...
      >
        ...
        >
          ...
          <span className="relative z-10 font-mono text-[0.5rem] tracking-[0.5em] text-sky-500/80 group-hover:text-sky-200 uppercase ml-[0.5em] font-light">
            {currentUser ? "▲ UPLINK NEW FRAGMENT" : "▲ LOG IN TO UPLINK"}
          </span>
          {CORNERS.map((cls, i) => (
