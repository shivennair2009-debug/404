...
/**
 * Deterministic per-fragment seed for SVG element variation.
 */
...
// ── Real-time glitched filter system for uploaded images ────────────────────
function CustomImageGlitched({ imageUrl, status }: { imageUrl: string; status: string }) {
  ...
  return (
    <div className="relative w-full h-full bg-[#020408] overflow-hidden flex items-center justify-center">
      ...
      {/* Slice Glitch Layer for CORRUPTED */}
      ...
      {/* Visual scanning line & color overlay */}
      {status === "RECOVERED" && (
...
function CorruptedVisual({ id, hasBg = true }: { id: string; hasBg?: boolean }) {
  ...
  return (
    <svg viewBox="0 0 200 140" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      ...
      {/* Central corruption focal point */}
      <motion.circle
        cx="100" cy="70" r="20"
...
function PartialVisual({ id, hasBg = true }: { id: string; hasBg?: boolean }) {
  ...
  return (
    <svg viewBox="0 0 200 140" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      ...
      {/* Focal point — partially resolved */}
      <motion.circle
        cx="100" cy="70" r="8"
      ...
      {/* Dissolve edge — right side fades */}
      {hasBg && (
...
function LostVisual({ id, hasBg = true }: { id: string; hasBg?: boolean }) {
  ...
  return (
    <svg viewBox="0 0 200 140" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      ...
      {/* Void center — dark pulsing circle */}
      <motion.circle
        cx="100" cy="70" r="25"
        fill="#020408"
        animate={{ r: [23, 28, 23] }}
...
export default function MemoryVisual({ id, status, imageUrl }: MemoryVisualProps) {
  ...
  return (
    ...
    >
      {/* Glitched Image Background if provided */}
      {imageUrl && (
      ...
      {/* SVG vector/glitch overlay effects */}
